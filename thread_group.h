...
CPU_ZERO(&AvailableCpusMask);
        ...
        std::FILE* cpuinfo = std::fopen("/home/rubis/Control_AirSim/log/coreinfo.txt", "w");
        ...
        if (num_accessible_cores > cores_for_unreal) {
            ...
            // unsigned long batch_size = thread_count / cores_for_unreal;
            for (auto &t : threads_) {
                // for (int i = 0; i < thread_count / batch_size; i++) {
                //     cpu_set_t mask_for_each;
                //     CPU_ZERO(&mask_for_each);
                //     CPU_SET(cores_for_unreal+i, &mask_for_each);
                //     if (t.native_handle() != 0) {
                //         pthread_setaffinity_np(t.native_handle(), sizeof(cpu_set_t), &mask_for_each);
                //     }
                // }
